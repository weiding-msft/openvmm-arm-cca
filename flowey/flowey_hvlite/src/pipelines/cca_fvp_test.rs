...
/// Build + run CCA FVP via Shrinkwrap and validate logs (local)
#[derive(clap::Args)]
pub struct CcaFvpTestCli {
    ...
    /// Rootfs path to pass at runtime
    #[clap(long)]
    pub rootfs: PathBuf,

    /// Additional runtime variables (repeatable), besides ROOTFS
    #[clap(long)]
    pub rtvar: Vec<String>,

    /// Extra args appended to `shrinkwrap build`
    #[clap(long)]
    pub build_arg: Vec<String>,

    /// Extra args appended to `shrinkwrap run`
    #[clap(long)]
    ...
    /// Regex marker required in console log for success
    #[clap(long)]
    pub boot_ok: String,
...
impl IntoPipeline for CcaFvpTestCli {
    fn into_pipeline(self, backend_hint: PipelineBackendHint) -> anyhow::Result<Pipeline> {
        if !matches!(backend_hint, PipelineBackendHint::Local) {
            anyhow::bail!("cca-fvp-test is for local use only");
        }
        ...
        pipeline
            .new_job(
                ...
                "cca-fvp-test: install shrinkwrap + build + run + validate",
            )
            ...
            // TODO: Add validation step that checks for boot_ok regex in console.log
            .finish();
