        ...
        if intercepted && this.runner.cca_rsi_plane_exit().exit_reason != RSI_PLANE_EXIT_INVALID {
            ...
            match exit_reason {
                PlaneExitReason::Sync => {
                    match cca_exit.esr_el2_class() {
                        ...
                        ExceptionClass::SystemRegister => {
                            ...
                            let exit_gprs = cca_exit.0.gprs.clone();
                            this.handle_system_register_trap(iss, source_value, exit_esr_el2, &exit_gprs, dev)?;
                        }
                ...
                PlaneExitReason::Irq => {
                    ...
                    let exit_gprs = cca_exit.0.gprs.clone();
                    this.request_asserted_local_interrupts(irq_exit, &exit_gprs, dev)?;
                }
...
impl UhProcessor<'_, CcaBacked> {
    ...
    fn handle_system_register_trap(
        ...
        let handled = self
            ...
            .cvm
            .gic
            .write_sysreg(system_reg, value, |target_vp| {
                tracing::trace!(
                    target_vp,
                    ?system_reg,
        ...
        if !handled {
            tracing::warn!(?system_reg, value, "unhandled GIC system-register write");
        }
    ...
    fn request_gic_interrupt(
        ...
        if !self.backing.vtls[vtl].gic.request_interrupt(intid) {
            return Err(
                dev.fatal_error(CcaUnsupportedExit::NoFreePendingGicInterrupt(intid).into())
            );
        }

        tracing::trace!(intid, ?vtl, "requested CCA private GIC interrupt");
        Ok(())
    }
    ...
    fn poll_gic(&mut self, vtl: GuestVtl) {
        loop {
            ...
            let Some(interrupt) = self.shared.cvm.gic.next_pending_interrupt(self.vp_index(), running_priority)
            else {
                return;
            };
            ...
            self.shared.cvm.gic.complete_interrupt(interrupt.intid, true);
